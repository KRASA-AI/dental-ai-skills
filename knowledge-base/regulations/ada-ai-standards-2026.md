...
Two further 2026 benchmarks surfaced this cycle sharpen the GlobalDentBench picture in a way that is directly relevant to how the repo's clinical skills are built. Both are academic studies of general-purpose models, so the same general-purpose-vs-cleared-device caveat above applies; they are cited here as concept-level evidence, not as claims about any named product.
...
Neither study changed any skill's prompt schema this cycle; together they strengthen the *rationale* behind the existing provider-concurrence, structured-intake, and citation guardrails. The net practical guidance is unchanged and reinforced: use the skills to gather and ground, let the clinician decide.
